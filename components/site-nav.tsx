    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 py-4 md:px-12">
          ...
          <div className="hidden md:flex items-center gap-6">
            <Link href="/solutions" className="group relative text-sm font-medium text-white/60 transition-colors hover:text-accent">
              Solutions
            ...
            <Link href="/blog" className="group relative text-sm font-medium text-white/60 transition-colors hover:text-accent">
              Blog
              <span className="absolute -bottom-1 left-0 h-px w-full origin-left scale-x-0 bg-accent transition-transform duration-300 group-hover:scale-x-100" />
            </Link>
            <Link href="/tarifs" className="group relative text-sm font-medium text-white/60 transition-colors hover:text-accent">
              Tarifs
              <span className="absolute -bottom-1 left-0 h-px w-full origin-left scale-x-0 bg-accent transition-transform duration-300 group-hover:scale-x-100" />
            </Link>
            ...
            <div ref={wrapRef} className="relative">
              <button
                ...
                className={`flex cursor-pointer items-center gap-1.5 text-sm font-medium transition-colors ${open ? "text-accent" : "text-white/60 hover:text-accent"}`}
                aria-expanded={open}
              >
                Démos
      ...
      >
        <div className="flex flex-col gap-2 px-6 pt-28 pb-10 flex-1 overflow-y-auto">
          ...
          {/* Démos par secteur */}
          <div className="pt-2">
            <p className="text-[10px] uppercase tracking-[3px] text-white/25 mb-3">Démos par secteur</p>
            {Object.entries(NICHES).map(([slug, niche]) => (
              <Link
                key={slug}
                href={`/demos/${slug}`}
